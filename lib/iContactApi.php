...
class iContactApi {
	...
	**/
	...
	public function getMessages($sType = null) {
		...
		return $this->makeCall("/a/228133/c/311256/sends", 'GET', null, 'messages');
	}

	public function getSendMessages($sType = null, $accountID) {
		// Check for a message type
		if (!empty($sType)) {
			$this->addCustomQueryField('messageType', $sType);
		}
		return $this->makeCall("/a/228133/c/".$accountID."/sends?status=released&limit=10", 'GET', null, null);
	}

	public function getMessagesInfo($messageid, $accountID) {
	
		return $this->makeCall("/a/228133/c/".$accountID."/messages/".$messageid, 'GET', null, null);
	}

	public function getMessagesStatistics($messageid, $accountID) {
	
		return $this->makeCall("/a/228133/c/".$accountID."/messages/".$messageid."/statistics", 'GET', null, null);
	}
